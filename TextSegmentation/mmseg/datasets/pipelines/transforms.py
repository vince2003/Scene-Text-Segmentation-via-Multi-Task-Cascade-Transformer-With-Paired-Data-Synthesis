...
class AlignedResize(object):
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg1_fields', []):
            if self.keep_ratio:
        ...
        for key in results.get('seg2_fields', []):
            if self.keep_ratio:
                gt_seg = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
                gt_seg = self._align(gt_seg, self.size_divisor, interpolation='nearest')
        ...
        for key in results.get('seg3_fields', []):
            if self.keep_ratio:
                gt_seg = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
                gt_seg = self._align(gt_seg, self.size_divisor, interpolation='nearest')
...
class Resize(object):
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg2_fields', []):
            if self.keep_ratio:
                gt_seg2 = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
            else:
                gt_seg2 = mmcv.imresize(
                    results[key], results['scale'], interpolation='nearest')
            results[key] = gt_seg2
        ...
        for key in results.get('seg3_fields', []):
            if self.keep_ratio:
                gt_seg3 = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
            else:
                gt_seg3 = mmcv.imresize(
                    results[key], results['scale'], interpolation='nearest')
            results[key] = gt_seg3        
        #pdb.set_trace()
...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        if self.cat_max_ratio < 1.:
            # Repeat 10 times
            for _ in range(10):
                seg_temp1 = self.crop(results['gt_semantic_seg1'], crop_bbox)                
                #seg_temp2 = self.crop(results['gt_semantic_seg2'], crop_bbox)
                
                labels, cnt = np.unique(seg_temp1, return_counts=True)
...
class MaillaryHack(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Processed results.
        """
        gt_map1 = results['gt_semantic_seg1']
        # others -> 255
        for value in self.others:
            gt_map1[gt_map1 == value] = 255

        for index, map in enumerate(self.map):
            for value in map:
                gt_map1[gt_map1 == value] = index

        results['gt_semantic_seg1'] = gt_map1
        
        #for ground truth 2
        
        gt_map2 = results['gt_semantic_seg2']
        # others -> 255
        for value in self.others:
            gt_map2[gt_map2 == value] = 255

        for index, map in enumerate(self.map):
            for value in map:
                gt_map2[gt_map2 == value] = index

        results['gt_semantic_seg2'] = gt_map2      
        
        #for ground truth 3
        
        gt_map3 = results['gt_semantic_seg3']
        # others -> 255
        for value in self.others:
            gt_map3[gt_map3 == value] = 255

        for index, map in enumerate(self.map):
            for value in map:
                gt_map3[gt_map3 == value] = index

        results['gt_semantic_seg3'] = gt_map3          
