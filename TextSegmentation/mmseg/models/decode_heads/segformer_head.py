...
class SegFormerHead(BaseDecodeHead):
    ...
    def __init__(self, **kwargs):
        ...
        params_seg=dict(input_transform='multiple_select', in_index=[0, 1, 2, 3],\
                        dropout_ratio=0.1, in_channels=[64, 128, 320, 512], feature_strides=[4, 8, 16, 32],\
                        num_classes=num_classes, decoder_params=768)
        self.Polygon = Polygon(**params_seg)
        ...
        self.skeleton_pred = nn.Conv2d(64, num_classes, kernel_size=1)
        

    def forward(self, inputs):
        #pdb.set_trace()
        seg_feat, detail_feat=self.SegHead(inputs) #([2, 64, 128, 128]), ([2, 64, 128, 128])

        #pdb.set_trace()
        polygon_feat, out_polygon=self.Polygon(inputs)
        
        
        skeleton_feat=self.Skeleton(inputs)#([2, 64, 128, 128]), torch.Size([2, 2, 512, 512])
        
        
        seg_feat=seg_feat*polygon_feat
        skeleton_feat=skeleton_feat*polygon_feat
        
        out_skeleton=self.skeleton_pred(skeleton_feat)
        
        

        
        glb_sup=seg_feat#+skeleton_feat.detach()
        ...
        #pdb.set_trace()
        return out, out_seg, out_polygon, out_skeleton
...
class Polygon(nn.Module):
    def __init__(self, input_transform, in_index, dropout_ratio, in_channels, feature_strides, num_classes, decoder_params):  
        ...
        self.linear_polygon = nn.Conv2d(embedding_dim, 64, kernel_size=1)
        self.linear_pred = nn.Conv2d(64, num_classes, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        x = self.linear_polygon(_c) 
        feat_polygon = x.clone()
