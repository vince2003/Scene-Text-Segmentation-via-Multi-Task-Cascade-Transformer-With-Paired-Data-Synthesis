...
class InpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.png', pad_out_to_modulo=None, scale_factor=None):
        ...
        self.mask_filenames = sorted(list(glob.glob(os.path.join(self.datadir, 'img_poly', '*.png'), recursive=True)))
        self.img_filenames = sorted(list(glob.glob(os.path.join(self.datadir, 'img', '*.png'), recursive=True)))
        self.masked_img_filenames = sorted(list(glob.glob(os.path.join(self.datadir, 'img_add_text', '*.png'), recursive=True)))
    ...
    def __getitem__(self, i):
        ...
        masked_image = load_image(self.masked_img_filenames[i], mode='RGB')
        
        
        name=self.img_filenames[i]      
        
        result = dict(image=image, masked_image=masked_image, mask=mask[None, ...], name=name)

        if self.scale_factor is not None:
            result['image'] = scale_image(result['image'], self.scale_factor)
            result['mask'] = scale_image(result['mask'], self.scale_factor, interpolation=cv2.INTER_NEAREST)
            result['masked_image'] = scale_image(result['masked_image'], self.scale_factor)

        if self.pad_out_to_modulo is not None and self.pad_out_to_modulo > 1:
            result['image'] = pad_img_to_modulo(result['image'], self.pad_out_to_modulo)
            result['mask'] = pad_img_to_modulo(result['mask'], self.pad_out_to_modulo)
            result['masked_image'] = pad_img_to_modulo(result['masked_image'], self.pad_out_to_modulo)
