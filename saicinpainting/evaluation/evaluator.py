        ...
        for score_name, score in tqdm.auto.tqdm(self.scores.items(), desc='scores'):
            ...
            with torch.no_grad():
                ...
                for batch in tqdm.auto.tqdm(self.dataloader, desc=score_name, leave=False):
                    ...
                    image_batch, masked_image_batch, mask_batch = batch['image'], batch['masked_image'], batch['mask']
                    if self.clamp_image_range is not None:
                        masked_image_batch = torch.clamp(masked_image_batch,
                    ...
                    if model is None:
                        ...
                        inpainted_batch = model(masked_image_batch, mask_batch)
...
class InpaintingEvaluatorOnline(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor]):
        ...
        result = {}
        with torch.no_grad():
            image_batch, masked_image_batch, mask_batch, inpainted_batch = batch[self.image_key], batch['masked_image'] ,batch['mask'], batch[self.inpainted_key]
            #pdb.set_trace()
            
            # show_dataloader(image_batch, 'image_batch_', normalize=True)  
            # show_dataloader(mask_batch, 'mask_batch_', normalize=True) 
            # show_dataloader(inpainted_batch, 'inpainted_batch_', normalize=True) 
