...
class MyDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #----------load_mask--------
        poly_path = path.replace('//train', '//train_poly')#[:-4]+"_mask000.png"
        mask = cv2.imread(poly_path)
        #print('before:',collections.Counter(mask.flatten()))

        #--------------------my customize-----------
        if (img.shape[0]+img.shape[1])>10000:
            img=cv2.resize(img,(5000,5000))
            mask=cv2.resize(mask,(5000,5000))
	    #name=fname.split('/')[-1]
	    #pdb.set_trace()
	    #print("testing",name)
	    #print("old shape",img.size)
    	   
	    #print("new shape",img.size)   
    #--------------------------------------------
        ...
        #--------load masked image------------
        masked_img_path = path.replace('//train', '//train_add_text')#[:-4]+".png"
        ...
        transformed = self.transform(image=masked_img, masks=mask_img)
        
        
        #---------get masked image transformed----------
        transformed_masked_img = transformed['image']
        transformed_masked_img = np.transpose(transformed_masked_img, (2, 0, 1))
        #-----------------------------------------------
        
        
        transformed_mask_img = transformed['masks']        
        #--------get mask transformed---------
        trans_mask =transformed_mask_img[0]
        transformed_mask = trans_mask[:,:,0]
        transformed_mask = np.expand_dims(transformed_mask, axis=0)/255
        transformed_mask = transformed_mask.astype('uint8')
        #---------------------------------------
        
        #---------get image transformed----------
        transformed_img =transformed_mask_img[1]
        transformed_img = np.transpose(transformed_img, (2, 0, 1))/255
     
        
        
        #print('after:',collections.Counter(transformed_mask.flatten()))
        
        #pdb.set_trace()
        #mask = self.mask_generator(transformed_image, iter_i=self.iter_i)
        #self.iter_i += 1
        return dict(masked_image=transformed_masked_img,
                    mask=transformed_mask,
                    image=transformed_img.astype(np.float32),
                    name=name)
...
class ImgSegmentationDataset(Dataset):
    def __init__(self, indir, mask_generator, transform, out_size, semantic_seg_n_classes=2):
        self.indir = indir
    ...
    def load_semantic_segm(self, img_path):
        ...
        segm_path = img_path.replace('//train', '//train_poly')[:-4]+"_mask000.png"
        mask = cv2.imread(segm_path, cv2.IMREAD_GRAYSCALE)/255
        mask = cv2.resize(mask, (self.out_size, self.out_size))
        tensor = torch.from_numpy(np.clip(mask.astype(int), 0, None))
...
def make_default_train_dataloader(indir, kind='mydataset', out_size=512, mask_gen_kwargs=None, transform_variant='default',
